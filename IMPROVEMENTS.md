...
**核心美学**：莫兰迪色系 × 大圆角 × 柔和阴影 × 优雅留白

采用极简柔和设计风格，打破传统企业系统的"严肃感"，营造友好、轻松的对话式体验。
...
| 用途 | 色值 | 说明 |
|------|------|------|
| 主色 | `#8B9FE8` | 柔和蓝紫，替代高饱和度蓝色 |
| 辅助色 | `#D4A5C9` | 淡紫点缀色 |
| 强调色 | `#A8D5BA` | 成功状态淡绿 |
| 基础背景 | `#FAFAFA` | 米白色，降低刺眼感 |
| 主文字 | `#2C3E50` | 深灰，减少对比度 |

**CSS 变量**：所有设计 Token 通过 CSS 变量定义（`--soft-primary`, `--bg-base`, `--text-primary` 等），便于主题扩展和暗色模式适配。

### 13.3 设计规范

- **圆角系统**：6px → 14-24px（卡片/容器使用大圆角，更柔和友好）
- **阴影系统**：使用蓝紫色调阴影替代黑色阴影，营造漂浮感
- **间距系统**：基于 8px 网格，卡片间距从 20px 提升到 24px，增加呼吸感
- **动画规范**：过渡时长 0.3s，缓动函数 `cubic-bezier(0.4, 0, 0.2, 1)`，性能优先（只用 transform/opacity）

### 13.4 关键改进对比

| 元素 | 优化前 | 优化后 |
|------|--------|--------|
| 主色 | `#165dff` (高饱和蓝) | `#8B9FE8` (柔和蓝紫) |
| 圆角 | 6px | 14-24px |
| 阴影 | 黑色阴影 | 蓝紫色调阴影 |
| 间距 | 20px | 24px |
| 动画 | 0.2s | 0.3s |
...
```
frontend/src/styles/
├── soft-design.css           # 全局设计系统（CSS 变量、Arco 覆盖、工具类）
├── query-page-optimized.css  # QueryPage 优化样式
└── animations.css            # 流畅动效系统

frontend/src/views/layout/MainLayout.vue  # 主布局优化
frontend/src/views/query/QueryPage.vue    # 查询页面优化
```
...
将前端导航从传统企业级复杂导航转换为**极简对话模式**：

1. **极简主义**：Agent 查询页面设为默认首页，侧边栏仅保留核心功能
2. **分层导航**：主工作区（高频对话）+ 快捷设置（下拉菜单）+ 管理中心（独立页面）
3. **极简柔和设计**：莫兰迪配色 + 大圆角 + 柔和阴影 + 流畅动效

### 14.2 路由变更

**原结构**：`/query`, `/agent-query`, `/semantic/*`, `/settings/*`, `/air/*`, `/can/*`, `/big/*`
...
/management (管理中心 - 独立布局)
├── /management/data/* (数据管理: datasources, datasets, metrics, dimensions, views, dictionaries)
├── /management/system/* (系统配置: model, query, lineage)
└── /management/advanced/* (高级模块: air, can, big)
```

### 14.3 导航层级设计

**主工作区侧边栏**：Logo → 新建对话 → 历史对话列表 → 用户头像+下拉菜单（个人设置、模型状态、管理中心、退出登录）

**管理中心页面**：独立布局，左侧导航分三大板块（数据管理、系统配置、高级模块），完全独立于主工作区

### 14.4 动效系统

实现了 16+ 种精心设计的动画效果：
- 侧边栏：历史项悬浮指示器动画、选中态淡入、删除按钮显隐
- 按钮：悬浮抬起 + 点击波纹 + 活跃缩放
- 用户头像：悬浮缩放 + 下拉菜单滑入
- 页面切换：淡出 → 滑入（方向感知）
- 卡片：入场滑入 + 悬浮抬起 + 列表依次淡入
- 加载：三点跳动 + 圆环旋转 + 骨架屏
- 特殊效果：玻璃态模糊 + 霓虹光晕 + 渐变边框

### 14.5 性能优化

- 只使用 GPU 加速属性（transform, opacity），避免重排
- 动效时长控制在 0.3-0.4s
...
```
frontend/src/views/management/ManagementLayout.vue  # 管理中心主布局（新增）
frontend/src/styles/animations.css                  # 流畅动效系统（新增）
frontend/src/router/index.ts                        # 路由配置重构
frontend/src/views/layout/MainLayout.vue            # 主布局重构为极简对话模式
frontend/src/main.ts                                # 引入动效样式
```
